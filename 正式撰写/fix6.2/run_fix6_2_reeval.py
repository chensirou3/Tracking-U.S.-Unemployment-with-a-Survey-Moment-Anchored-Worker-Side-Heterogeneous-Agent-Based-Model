"""
Section 6.2 fix — re-evaluate top-5 calibrated candidates per variant on 5 final seeds.

Outputs:
  正式撰写/fix6.2/reeval_trajectories.npz   per-variant per-cand per-seed UR/LFPR/EPOP
  正式撰写/fix6.2/reeval_metrics.json        per-window metrics for every (variant, cand, seed)
"""
...
# Reuse builders from the calibration script
sys.path.insert(0, str(Path('正式撰写/fix6.2').resolve()))
from run_fix6_2_calibrate import run_one, param_names, VARIANT_SPEC  # noqa: E402

OUT = Path('正式撰写/fix6.2')
SEEDS = [42, 137, 2024, 888, 1234]

# Same window definitions as fix6.1 (half-open [s, e))
WINDOWS = {
...
def main():
    ...
    target_ur = np.array(env.target_ur if hasattr(env, 'target_ur') else [])
    # RealEnvironment doesn't expose targets directly; load via calibration_engine
    from Phase3_Code.calibration_engine import target_ur as t_ur, target_lfpr as t_lf, target_epop as t_ep

    # Storage: (variant, cand_rank, seed) -> 302-length arrays
    traj = {}
    metrics = {}
    for variant in VARIANT_SPEC.keys():
        ...
        v_metrics = []  # list of (rank, cand_idx, seed, window) dicts
    ...
    print(f"Wrote: {OUT / 'reeval_trajectories.npz'}")
    print(f"Wrote: {OUT / 'reeval_metrics.json'}")
